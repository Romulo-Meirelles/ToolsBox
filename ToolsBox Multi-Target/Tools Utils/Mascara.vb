...
Namespace Utils
    Public Module Mascara
        ...
        Public Function MaskDollar(CaixaTexBox As Object)
            On Error Resume Next
            Dim C As Decimal = CInt(CaixaTexBox.Text.Replace(",", "."))
            Return C.ToString("C2", CultureInfo.CreateSpecificCulture("en-US"))
        End Function

        Public Sub MaskDollar_ChangedText(CaixaTexBox As Object)
            On Error Resume Next
            Dim n As String = String.Empty
            Dim v As Double = 0
            n = CaixaTexBox.Text.Replace(",", "").Replace(".", "").Replace("$", "")
            v = Convert.ToDouble(n) / 100
            CaixaTexBox.Text = v.ToString("C2", CultureInfo.CreateSpecificCulture("en-US"))
            CaixaTexBox.SelectionStart = CaixaTexBox.Text.Length
        End Sub

        Public Function MaskEuro(CaixaTexBox As Object)
            On Error Resume Next
            Dim C As Decimal = CInt(CaixaTexBox.Text.Replace(",", "."))
            Return C.ToString("C2", CultureInfo.CreateSpecificCulture("fr-FR"))
        End Function

        Public Sub MaskEuro_ChangedText(CaixaTexBox As Object)
            On Error Resume Next
            Dim n As String = String.Empty
            Dim v As Double = 0
            n = CaixaTexBox.Text.Replace(",", "").Replace(".", "").Replace("€", "")
            v = Convert.ToDouble(n) / 100
            CaixaTexBox.Text = v.ToString("C2", CultureInfo.CreateSpecificCulture("fr-FR"))
            CaixaTexBox.SelectionStart = CaixaTexBox.Text.Length
        End Sub
