...
Public Class HorizontalBar
    ...
    Public Event Click_HorizontalBar(sender As Object, segment As PyramidSegment, index As Integer)


    ' ===== PROPRIEDADES =====
    <Category("HorizontalBar"), DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)>
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        For i As Integer = 0 To _segments.Count - 1
            ...
            ' ===== ANIMAÇÃO HOVER =====
            Dim hoverOffset As Single = -seg.HoverProgress * 8.0F

            ' ===== RETÂNGULO DO SEGMENTO =====
            Dim y As Single = rect.Top + i * stepH + _segmentSpacing / 2 + hoverOffset
            Dim h As Single = stepH - _segmentSpacing

            Dim segRect As New RectangleF(
            rect.Left,
            y,
            rect.Width,
            h
            ...
            ' ===== PATH (HIT TEST + CLICK + HOVER) =====
            Dim path As New GraphicsPath()
            path.AddRectangle(segRect)
            _segmentPaths.Add(CType(path.Clone(), GraphicsPath))

            ' ===== DESENHO =====
            Using brSeg As New SolidBrush(seg.FillColor)
                g.FillRectangle(brSeg, segRect)
            End Using
            ...
            If showText OrElse showPct Then
                ...
                Dim ty As Single = segRect.Top + (segRect.Height - blockHeight) / 2
                ...
                If showText Then
                    Using brTxt As New SolidBrush(seg.InsideTextColor)
                        ...
                        seg.InsideTextFont,
                        brTxt,
                        New RectangleF(segRect.X, ty, segRect.Width, seg.InsideTextFont.Height),
            ...
            If needRightSpace AndAlso (seg.ShowPointer OrElse seg.ShowOutsideText) Then
                ...
                Dim midY As Single = segRect.Top + segRect.Height / 2
                Dim xStart As Single = segRect.Right
    ...
    '    ' ───── VERIFICA SE PRECISA ESPAÇO LATERAL ─────
    '    Dim needRightSpace As Boolean =
    '    _segments.Any(Function(s) s.ShowPointer OrElse s.ShowOutsideText)

    '    Dim rightPadding As Integer = If(needRightSpace, 90, 0)
    ...
    '    '    rect.X,
    '    '    rect.Y + i * stepH + _segmentSpacing / 2,
    '    '    rect.Width,
    '    '    stepH - _segmentSpacing
    '    ')
    ...
    '    '        Dim y As Single = r.Top + (r.Height - blockHeight) / 2
    ...
    '    '                seg.InsideTextFont,
    '    '                brTxt,
    '    '                New RectangleF(r.X, y, r.Width, seg.InsideTextFont.Height),
    '    '                New StringFormat With {
    '    '                    .Alignment = StringAlignment.Center,
    '    '                    .LineAlignment = StringAlignment.Center
    '    '                }
    ...
    '    '        Dim midY As Single = r.Top + r.Height / 2
    '    '        Dim xStart As Single = r.Right + 4
    '    '        Dim xEnd As Single = Width - 8
    ...
    '    Dim centerX As Single = rect.Left + rect.Width / 2
    '    Dim stepH As Single = rect.Height / _segments.Count
    '    Dim total As Decimal = _segments.Sum(Function(s) s.Value)

    '    ' ===== DESENHO =====
    '    For i As Integer = 0 To _segments.Count - 1

    '        Dim seg As PyramidSegment = _segments(i)

    '        Dim expand As Single =
    '        If(_enableHoverAnimation AndAlso i = _hoverIndex, _hoverExpand, 0)

    '        Dim ratioTop As Single = i / _segments.Count
    '        Dim ratioBottom As Single = (i + 1) / _segments.Count

    '        Dim topW As Single = rect.Width * ratioTop + expand
    '        Dim botW As Single = rect.Width * ratioBottom + expand

    '        Dim y1 As Single = rect.Top + i * stepH + _segmentSpacing / 2
    '        Dim y2 As Single = y1 + stepH - _segmentSpacing
    ...
    '            ' ==================================================
    '            ' TEXTO INTERNO + PORCENTAGEM (LAYOUT CORRETO)
    '            ' ==================================================
    '            Dim hasText As Boolean = seg.ShowInsideText AndAlso Not String.IsNullOrEmpty(seg.InsideText)
    '            Dim hasPct As Boolean = seg.ShowPercentage AndAlso total > 0

    '            If hasText OrElse hasPct Then

    '                Dim elements As Integer = If(hasText, 1, 0) + If(hasPct, 1, 0)
    '                Dim centerY As Single = bounds.Top + bounds.Height / 2

    '                ' ----- TEXTO -----
    '                If hasText Then
    '                    Dim yText As Single =
    '                    If(elements = 2,
    '                       centerY - seg.InsideTextFont.Height,
    '                       centerY - seg.InsideTextFont.Height / 2)
    ...
    '                        seg.InsideTextFont,
    '                        brTxt,
    '                        New RectangleF(bounds.X, yText, bounds.Width, seg.InsideTextFont.Height),
    '                        New StringFormat With {
    '                            .Alignment = StringAlignment.Center,
    '                            .LineAlignment = StringAlignment.Center
    '                        }
    ...
    '                ' ----- PORCENTAGEM -----
    '                If hasPct Then
    '                    Dim pct As String = $"{Math.Round((seg.Value / total) * 100, 1)}%"
    '                    Dim yPct As Single =
    '                    If(elements = 2,
    '                       centerY,
    '                       centerY - seg.InsideTextFont.Height / 2)
    ...
    '                        seg.InsideTextFont,
    '                        brTxt,
    '                        New RectangleF(bounds.X, yPct, bounds.Width, seg.InsideTextFont.Height),
    '                        New StringFormat With {
    '                            .Alignment = StringAlignment.Center,
    '                            .LineAlignment = StringAlignment.Center
    '                        }
    ...
    '            ' POINTER + TEXTO EXTERNO
    '            ' ==================================================
    '            If seg.ShowPointer OrElse seg.ShowOutsideText Then

    '                Dim midY As Single = (y1 + y2) / 2
    '                Dim xStart As Single = centerX + botW / 2
    '                Dim xEnd As Single = rect.Right - 10
    ...
    '                        midY - seg.OutsideTextFont.Height - 4,
    ...
    Protected Overrides Sub OnMouseDown(e As MouseEventArgs)
        ...
        For i As Integer = 0 To _segmentPaths.Count - 1
            If _segmentPaths(i).IsVisible(e.Location) Then
                RaiseEvent Click_HorizontalBar(Me, _segments(i), i)
                Exit For
            End If
        Next
    End Sub
